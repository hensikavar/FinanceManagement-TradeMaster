...
const Paymentout = () => {
  ...
  //setstates
  const [payoutpartyname,setpayoutpartyname] = useState("");
  const [payoutpartyphoneno,setpayoutpartyphoneno] = useState("");
  const [payoutpaymenttype,setpayoutpaymenttype] = useState("");
  const [payoutdescription,setpayoutdescription] = useState("");
  const [payoutpaymentrefno,setpayoutpaymentrefno] = useState("");
  const [payoutdate , setpayoutdate] = useState("");
  const [payoutrec , setpayoutrec] = useState("");
  ...
  const [showpayoutBillMaker,setShowpayoutBillMaker] = useState(false);
  // const [showAddItem,setShowAddItem] = useState(false);
  const [showrefno , setShowrefno] = useState(false);
  const [paymentouttransaction,setPaymentouttransaction]=useState([]);
  ...
  const postpayoutbillData = ()=>{
    ...
    fetch("http://localhost:5000/paymentout/",{
        ...
        body:JSON.stringify({
          payout_partyname:payoutpartyname,
          payout_phonenumber:payoutpartyphoneno,
          payout_date:payoutdate,
          payout_description:payoutdescription,
          payout_payment_ref_no:payoutpaymentrefno,
          payout_paymenttype:payoutpaymenttype,
          payout_received:payoutrec,
        })
    })
    .then(res=>res.json())
    .then((data)=>{
      ...
      if(data.error){
        ...
        // setSalebilltransaction([...salebilltransaction, data]);
          // setSaleBillItems([]);
          setpayoutpartyname("");
          setpayoutpartyphoneno("");
          setpayoutdate("");
          setpayoutdescription("");
          setpayoutpaymenttype("");
          setShowpayoutBillMaker(false);
          fetchPaymentoutTransactions();
          // setSalebilltransaction([...salebilltransaction, data]);
        //   fetchParties();
      }
    })
  ...
  const fetchPaymentoutTransactions = () => {
    // Fetch updated sale bill transactions
    fetch("http://localhost:5000/paymentout")
      .then(res => res.json())
      .then(data => {setPaymentouttransaction(data);console.log("data came")})
      .catch(error => console.error('Error fetching payment out transactions:', error));
  }
  useEffect(() => {
    fetchPaymentoutTransactions();
  }, []);
  ...
  const filteredpayouttran = paymentouttransaction.filter(sale =>
    sale.payout_partyname.toLowerCase().includes(searchQuery.toLowerCase()) 
    // &&
    // (new Date(sale.payout_date) >= new Date(startDate) &&
  ...
  const totalamt = paymentouttransaction.reduce((total, transaction) => total + transaction.payout_total_amt, 0) || 0;
  return (
    <div>
      <h2>Payment in</h2>
       {showpayoutBillMaker && (
                ...
                <div className='input-grp '>
                          <div className="col-8">
                            <div className="name-phone">
                              <div className='input-fld'>
                                  <select className="type" value={payoutpartyname} onChange={(e) => { setpayoutpartyname(e.target.value) }} style={{height:"50px",textAlign:"left"}}>
                              ...
                              <div className='input-fld pt-1'>
                                  <input type='text' value={payoutpartyphoneno} onChange={(e)=>{setpayoutpartyphoneno(e.target.value)}} placeholder='Phone Number'/>
                              </div>
                              {/*  */}
                            </div>
                          </div>
                          <div className="col-4">
                            <table className="item-list">
                              ...
                              <tr>
                                <td>Date  </td>
                                <td>:</td>
                                <td><input type='date' value={payoutdate} onChange={(e)=>{setpayoutdate(e.target.value)}} placeholder='date'/></td>
                              </tr> 
                            </table>
                          </div>
                      ...
                      <div className='input-grp' style={{justifyContent:"space-between"}}>
                          ...
                          <div className='input-fld-s col-9' style={{ display:"flex",padding: "13px", width: "350px" }}>
                              ...
                              <div style={{ marginLeft: "10px" }}>
                                  <select className='type' style={{ width: "200px" }} value={payoutpaymenttype} onChange={(e) => { setpayoutpaymenttype(e.target.value); checkType(e.target.value); }}>
                          ...
                          {showrefno && payoutpaymenttype === "cheque" && (
                              <div className='input-fld-s' style={{padding:"3px", width: "200px" }}>
                                  {/* <div>Reference no :</div> */}
                                  <div><input type="text" value={payoutpaymentrefno} onChange={(e)=>{setpayoutpaymentrefno(e.target.value)}}  placeholder="enter Ref no"/></div>
                              </div>
                          )}
                          <div className="col-4">
                            <table className="item-list">
                              <tr>
                                <td>Received.</td>
                                <td>:</td>
                                <td><input type='text' value={payoutrec} onChange={(e)=>{setpayoutrec(e.target.value)}} placeholder='Received amt'/></td>
                              </tr>
                            </table>
                      ...
                      <div className='input-grp-s'style={{justifyContent:"space-between"}}>
                          <div className='input-fld-s col-9'style={{display:"flex",padding:"10px", width:"60%", marginTop:"30px"}}>
                            <div>decription : </div>
                            <div style={{width:"80%"}}><input type="text" value={payoutdescription} onChange={(e)=>{setpayoutdescription(e.target.value)}} placeholder="description"/></div>
                          </div>
                      
                      <div className='input-fld-s col-3' style={{display:"flex",padding:"10px", width:"200px"}}>
                              <div>total : </div>
                              <div style={{ width:"70%"}}>{payoutrec}</div>
                      </div>
                      </div>
      ...
      <div className='row main_ps_page'>
        ...
        <div className='ps-tranc-out'>
            ...
            <div className='ps-search'>
              ...
              <input type='button' className='add-tran btn btn-primary' onClick={()=>{setShowpayoutBillMaker(true)}} value={"Add Payment out"}/>
            </div>
            <div className='ps-tranc-table'>
              <table class="table table-hover">
                ...
                <tbody>
                {filteredpayouttran.map((transaction, index) => (
                    <tr key={index}>
                      {/* <td scope="row">{transaction.sb_date}</td> */}
                      <td>{index+1}</td>
                      <td>{transaction.payout_partyname}</td>
                      <td>{transaction.payout_paymenttype}</td>
                      <td>{transaction.payout_paymenttype == "cheque"?transaction.payout_payment_ref_no:"-"}</td>
                      <td>{transaction.payout_total_amt}</td>
                      <td>{transaction.payout_received}</td>
                    </tr>
                  ))}
